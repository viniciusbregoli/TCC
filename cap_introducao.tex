...
A \textit{mineração de processos} (PM) consolida-se como abordagem para extração de conhecimento estruturado a partir de logs de eventos, com aplicações demonstradas em domínios complexos. Entretanto, PM oferece predominantemente uma visão descritiva e diagnóstica. A \textit{simulação de eventos discretos} (DES), por outro lado, permite avaliar cenários alternativos, mas sua construção manual é intensiva e inviável para decisões de curto prazo que exigem reação rápida a eventos inesperados.
...
\begin{itemize}
	...
	\item \textbf{Implementar} protótipo funcional integrado em pipeline único, com tempo de execução inferior a 5 minutos para logs típicos (10.000+ eventos);
...
\textbf{Capítulo 2 – Referencial Teórico:} Apresenta fundamentos de mineração de processos, simulação de eventos discretos, integração PM-DES, e análise crítica do estado da arte com identificação de lacunas que motivam o trabalho.

\textbf{Capítulo 3 – Metodologia:} Descreve arquitetura do sistema proposto, tecnologias utilizadas (PM4PY, SimPy), fluxo do pipeline, protocolos de parametrização estatística e estratégias de validação.

\textbf{Capítulo 4 – Desenvolvimento e Implementação:} Detalha decisões de design, componentes do protótipo, algoritmos de descoberta de processos, extração de distribuições, geração de código SimPy e integração end-to-end.

\textbf{Capítulo 5 – Validação Experimental:} Apresenta estudo de caso hospitalar, métricas de avaliação (fitness, precisão, desempenho), análise de resultados e discussão de limitações.

\textbf{Capítulo 6 – Conclusões e Trabalhos Futuros:} Sintetiza contribuições, impactos potenciais, limitações identificadas e perspectivas de evolução tecnológica.
